...
<div class="flex flex-col gap-4">
  ...
  {#if label}
    <div class="flex flex-col gap-1">
      <span class="text-xs text-gb-light-fg3 dark:text-gb-fg3 uppercase tracking-wider">Label</span>
      <p class="text-sm text-gb-light-fg dark:text-gb-fg">{label}</p>
    </div>
  {/if}
    ...
    <div class="flex flex-col gap-1.5">
      ...
      <div class="flex flex-col gap-1">
        {#each activeTasks as task (task.id)}
          <span class="text-sm flex items-center gap-2 {completedTaskIds.has(task.id) ? 'text-gb-light-fg dark:text-gb-fg' : 'text-gb-light-fg3 dark:text-gb-fg3'}">
  ...
  <div class="flex flex-col gap-1">
    ...
    {:else}
      <p class="text-gb-light-fg3 dark:text-gb-fg3 text-sm italic">No notes yet.</p>
    {/if}
  </div>
